...
You are now ready to build a `docker` image for `machine-operator-monitor-go` application and stored it in the ACR you had built earlier. We assume you have already cloned the `machine-operator-monitor-go` git repository as per instructions in [README](./README.md).
...
```
az acr build --resource-group myResourceGroup --registry myOpenVinoGocv --image machine-operator-monitor-go .
```
...
- image:
    ...
    repository: machine-operator-monitor-go
    tag: latest
...
```
docker run -it --rm myopenvinogocv.azurecr.io/machine-operator-monitor-go -h
```
